...
func (s *Store) ReviewEvents(ctx context.Context, req EventReviewRequest) (EventReviewPage, error) {
 ...
 LEFT JOIN LATERAL (SELECT jsonb_build_object('attempt_id',n.attempt_id,'state',CASE WHEN n.finished_at IS NULL THEN 'running' ELSE 'finished' END,'created_at',n.created_at,'finished_at',n.finished_at,'reason',n.reason,'session',jsonb_build_object('agent_id',n.agent_id,'execution_id',n.execution_id)) AS detail FROM cairn.agent_session_attempt n WHERE n.delivery_id=d.delivery_id ORDER BY n.created_at DESC,n.attempt_id DESC LIMIT 1) n ON true
