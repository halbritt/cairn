...
func (s *Store) Recompile(ctx context.Context, req RecompileRequest) (Package, error) {
	...
	for _, e := range evaluations {
		if e.Facts == nil {
			switch e.Reason {
			case "CLASS_NOT_CONSEQUENTIAL", "AUTHORITY_INACTIVE", "POLICY_UNENFORCEABLE", "OPEN_CONFLICT", "ATTRIBUTION_UNRECONCILED", "EVIDENCE_UNAVAILABLE", "CONTEXT_MISSING", "CURRENTNESS_MISMATCH", "OUTSIDE_VALIDITY":
				p.Omitted[e.Reason]++
				continue
			default:
