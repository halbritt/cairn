...
func (s *Store) ClaimSessionInbox(ctx context.Context, req SessionInboxClaim, dest Destination) (SessionInboxResult, error) {
	...
	var owner, agentID, executionID string
	var prior *string
	err = tx.QueryRow(ctx, `SELECT owner,agent_id::text,execution_id::text,attempt_id::text FROM cairn.agent_session_poll WHERE request_id=$1`, req.RequestID).Scan(&owner, &agentID, &executionID, &prior)
	if err == nil {
	...
	commit := func(attempt *SessionInboxAttempt) (SessionInboxResult, error) {
		...
		if _, err := tx.Exec(ctx, `INSERT INTO cairn.agent_session_poll(request_id,agent_id,execution_id,attempt_id) VALUES($1,$2,$3,$4)`, req.RequestID, req.Session.AgentID, req.Session.ExecutionID, id); err != nil {
			return out, err
		}
		return SessionInboxResult{attempt}, tx.Commit(ctx)
	}
	...
	if _, err = tx.Exec(ctx, `INSERT INTO cairn.agent_session_attempt(attempt_id,agent_id,execution_id,delivery_id,lease_id) VALUES($1,$2,$3,$4,$5)`, req.RequestID, a.AgentID, a.ExecutionID, id, lease); err != nil {
		return out, err
	}
	attempt, err := s.readSessionInboxAttempt(ctx, tx, req.RequestID, req.Session, dest)
	if err != nil {
