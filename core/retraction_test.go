...
func TestRetractionPreviewInvalidationAndRetry(t *testing.T) {
	...
	compile := CompileRequest{uuid.NewString(), Scope{repo, "task", "run"}, "fixture_error", "context", 64000}
...
func TestConcurrentCompileAndRetraction(t *testing.T) {
	...
	for range 10 {
		...
		go func() {
			<-start
			p, err := s.Compile(ctx, CompileRequest{uuid.NewString(), Scope{repo, "task", "run"}, "fixture_error", "context", 64000}, Destination{"local", true})
