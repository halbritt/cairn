...
func validateFrozenDiscovery(p SemanticPackage, evaluations map[string]*CandidateEvaluation, query string) error {
	...
	if p.Schema != "cairn.semantic/7" && (p.Schema != "cairn.semantic/8" || p.Discovery == nil) {
	...
	if d.State != "ready" {
		...
		for _, e := range evaluations {
			if e.SemanticScore != nil || (d.State == "not_needed" && e.Facts != nil && !e.Mandatory) {
				return invalid()
			}
		}
		return nil
