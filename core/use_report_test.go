...
func TestUseReportJoinsWithoutMultiplyingObservations(t *testing.T) {
	...
	req := CompileRequest{uuid.NewString(), Scope{repo, "task", "run"}, "", "context", 64000}
...
func TestRunAssessmentIsVersionedAndDoesNotInferCapabilityFromBinding(t *testing.T) {
	...
	p, err := s.Compile(ctx, CompileRequest{uuid.NewString(), Scope{repo, "task", "run"}, "", "context", 64000}, Destination{"local", true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestUsageInferenceAndCoverageRemainExplicit(t *testing.T) {
	...
	p, err := s.Compile(ctx, CompileRequest{uuid.NewString(), Scope{repo, "t", "r"}, "", "context", 64000}, Destination{"local", true})
	if err != nil {
		t.Fatal(err)
	}
