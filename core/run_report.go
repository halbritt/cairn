...
func (s *Store) RunReport(ctx context.Context, req RunReportRequest) (RunReport, error) {
	...
	rows, err := tx.Query(ctx, `SELECT r.receipt_id::text,r.scope,r.created_at,r.launch_claimed,b.receipt_id IS NOT NULL,o.receipt_id IS NOT NULL,
 ...
 COALESCE(a.version,0),COALESCE(a.witness,'unknown'),COALESCE(a.detail->>'method',''),COALESCE(a.failure_domain,'unknown'),COALESCE(a.failure_kind,''),COALESCE(a.detail->>'error_signature_sha256','')
 ...
 ORDER BY r.created_at,r.receipt_id LIMIT $2 OFFSET $3`, req.Repo, req.Limit+1, req.Offset)
	if err != nil {
		return RunReport{}, err
	}
