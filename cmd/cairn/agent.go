...
func agentRequest(ctx context.Context, args []string, input io.Reader) (any, error) {
	...
	operation := f.Arg(0)
	switch operation {
	case "check-evidence", "refusal", "index", "expand", "create", "edit", "delete", "compile", "get", "usage", "usage-coverage", "evidence", "spawn", "terminal", "task-state", "bind-run", "claim-run", "delivery", "outcome", "assess-run", "use-report", "run-report", "conflict", "conflicts":
	default:
