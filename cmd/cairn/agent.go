...
func agentRequest(ctx context.Context, args []string, input io.Reader) (any, error) {
	...
	switch operation {
	case "event-watch":
	case "agent-register", "agent-context", "agent-heartbeat", "agent-leave", "agent-directory", "agent-resolve", "session-inbox-ready", "session-inbox-claim", "session-inbox-reconcile":
