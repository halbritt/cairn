...
// installOpenCode writes only Cairn's adapter and connection file. It does not
...
func installOpenCode(args []string, executable string) (openCodeInstallation, error) {
	...
	for _, path := range []string{directory, tools} {
	...
	// Check both destinations before changing either file. Existing customizations
	...
	for i := range files {
		...
		if !bytes.Equal(old, file.body) && !*replace {
			return result, invalid(file.path + " differs; inspect it and use --replace to replace both Cairn files")
		}
		file.same = bytes.Equal(old, file.body) && info.Mode().Perm() == 0600
	}
	if err = os.MkdirAll(tools, 0700); err != nil {
		return result, installationError(tools, err)
	}
