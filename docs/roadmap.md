...
addresses these boundaries. The owner's follow-up ordered selective recall,
separate durable notes, named workstreams, unchanged-capture suppression, then
OpenCode integration and Codex/Agy hook assessment. These slices now share one
engine; see [OpenCode lifecycle memory](opencode-lifecycle.md),
[verification](verification/lifecycle-improvements-2026-09-13.md), and the
...
| U5 — usage ladder | partial | Preserve cited > expanded > behaviorally implicated > delivered-only, with inference method/version and observation coverage. Connect actual citation/expansion ingress; manual testimony does not prove use. The [expansion observation field](verification/expansion-observation-2026-09-10.md) keeps retained service-side expansion visible alongside later testimony or citation, without implying response delivery or benefit. Do not infer unknown H0 tool actions or benefit from delivery. Test coverage-aware reporting. |
...
| ID | Status | Work and acceptance evidence |
| --- | --- | --- |
| E1 — currentness and intent | partial | Declared repository revision/dirty-workspace identity, validity, task class/phase and capability/binding pins are implemented. [Task phases](currentness-and-replay.md#declared-task-phases) now narrow guidance within one task class across CLI, MCP and native OpenCode; missing mandatory context, inheritance, retained-run intent and old receipt seals are verified. Ordinary CLI/MCP/OpenCode capture accepts explicit applicability pins without inheriting search context. Native MCP/OpenCode capture now also accepts explicit repository/task/run scope using host search labels, preserving repository defaults. [Capture scope checks](verification/native-capture-scope-2026-09-10.md) cover cross-session retrieval and unrelated-task omission; useful downstream task continuity still needs observation. All database readers must be upgraded before phase constraints are written. [Quoted search](quoted-search.md) now prefers exact body text for supplied identifiers, paths and error messages, without changing eligibility. [Reviewed failure signatures](failure-signature-search.md) now retrieve exact converted lesson versions through CLI/MCP/OpenCode, with explicit association sharing and historical replay. Explicit filename/symbol lookup is available through quoted body text and is now explained in repository agent instructions. This does not resolve typed entity identity or observe a file being touched. [Explicit versioned file/symbol associations](entity-search.md) now support exact typed matching through ordinary capture/search/edit/history and retained replay. Names are repository-relative file paths or opaque qualified symbols, with no rename/alias inference. [Optional OpenCode recent-file hints](recent-file-hints.md) now collect successful native text-file reads for fresh permission-checked searches. Explicit hints, retries/pages and capture remain under caller control. This is bounded session-local file intent; automatic intent in other harnesses, symbols and sustained task-value evidence remain open. Executable currentness predicates are an optional later experiment. Continue testing obsolete guidance across code revisions and mismatched scopes. |
...
| E3 — cutoff-pinned recompilation | partial | [Retained-read-set recompilation is implemented and tested](verification/replay-requirements-2026-09-10.md): the named receipt fixes original candidates, versions, scope/context, policy, gate facts, ranking and budgets. Later-created advice and required instructions, corrections, revocations, evidence changes and conflict resolution do not rewrite that result; unavailable payloads remain unavailable. Saved-byte replay remains inspection. [Authenticated historical reconstruction](verification/authenticated-recompile-2026-09-10.md) now checks receipt ownership, original destination, current record privacy and forgetting before returning an unchanged package. The actual retained host receipt recompiled with its original observer token and excluded the later failure lesson; this supersedes the preceding audit's access gap. The remaining evaluation gap is an original-incident case with verified pre-cutoff memory/evidence and historical repository revision, distinct from a later recurrence; existing Striatum coverage imported its notes later and does not establish this. Keep that evidence requirement alongside E4; do not infer a need for a universal temporal store or reconstruct an unrecorded past from digests. |
...
matching and [explicit file/symbol associations](entity-search.md) are implemented.
Arbitrary historical cutoffs without retained observations, automatic host intent
collection and broader real-history usefulness remain open.
...
| ID | Status | Work and acceptance evidence |
| --- | --- | --- |
| L1 — class-proportional lifecycle | implemented | Ordinary revisions, [unreferenced A deletion](ordinary-delete.md), cheap B→A and [cross-record supersession](supersession.md) preserve retained history and apply their class-specific guards. [Scope authorization](scope-authorization.md) now expands applicability within a repository through an audited C decision, retains independent B/C qualification and supports explicit scope reauthorization. Tests cover surviving history, known impact notices, authority/dependency/refusal boundaries, concurrent CAS and backup/restore. The [ordinary retirement guide](ordinary-delete.md#retire-obsolete-advice-while-retaining-history) explains the existing grant-backed retraction path for used A notes; ordinary hosted tools do not expose retirement. This does not close the separate governance, deletion, retention or restore-admission gates below. |
| L2 — conflicts and refusal records | partial | [Conflict inspection](conflict-inspection.md) now exposes original versioned positions, participation and resolution history, with scope/destination checks and forgotten-body exclusion. [Qualified advisory positions](advisory-conflicts.md) now support explicit context retrieval with complete competing groups, private-counterpart omission, bounded index/pull allocation, currentness checks and frozen historical recompilation. Richer interested-party resolution and acted-under-open-conflict outcomes remain open. Durable policy refusals exist. Whole-group omission of optional disputed content remains allowed; closing a group preserves its original context. |
