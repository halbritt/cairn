// Native conversation identity/presence and exclusive request admission bridge.
          ...
          try {
            if (method === "session/prompt_async") {
              ...
              if (typeof client_id !== "string" || !client_id.trim()) {
                socket.write(JSON.stringify({ id, error: { code: -32602, message: "client_id required" } }) + "\n")
                continue
              }
              if (!instClient?.session?.promptIdle || !instClient?.session?.cancelRequest) {
                socket.write(JSON.stringify({ id, error: { code: -32004, message: "UNSUPPORTED_CONTROL: patched native request ownership is required" } }) + "\n")
                continue
              }
              const promptRes = await instClient.session.promptIdle({
                path: { id: session_id },
                body: { requestID: client_id, parts: [{ type: "text", text }] },
              })
              if (promptRes?.error) {
                const missing = promptRes.response?.status === 404
                socket.write(JSON.stringify({ id, error: {
                  code: missing ? -32002 : -32000,
                  message: missing ? "SESSION_NOT_FOUND" : `PROMPT_REFUSED: ${JSON.stringify(promptRes.error)}`,
                } }) + "\n")
                continue
              }
              const outcome = promptRes?.data?.status
              if (outcome === "busy") {
                socket.write(JSON.stringify({ id, error: { code: -32600, message: "BUSY: native ownership refused admission" } }) + "\n")
                continue
              }
              if (outcome !== "accepted" && outcome !== "completed") {
                socket.write(JSON.stringify({ id, error: { code: -32000, message: `PROMPT_REFUSED: ${outcome}` } }) + "\n")
                continue
              }
              ...
              const { session_id, expected_turn_id } = params
              if (typeof session_id !== "string" || !session_id || typeof expected_turn_id !== "string" || !expected_turn_id) {
                socket.write(JSON.stringify({ id, error: { code: -32602, message: "session_id and expected_turn_id required" } }) + "\n")
                continue
              }
              ...
              const result = await instClient.session.cancelRequest({ path: { id: session_id }, body: { requestID: expected_turn_id } })
              if (result?.error || typeof result?.data?.cancelled !== "boolean") {
                socket.write(JSON.stringify({ id, error: { code: -32000, message: "CANCEL_OUTCOME_UNCERTAIN" } }) + "\n")
                continue
              }
              socket.write(JSON.stringify({ id, result: { aborted: result.data.cancelled } }) + "\n")
