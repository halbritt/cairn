...
export const edit = validatedTool({
  ...
  async execute(args, context) {
    if ([args.body, args.append, args.draft, args.evidence_citations].filter(value => value !== undefined).length !== 1) throw new Error("INVALID_REQUEST: supply exactly one of body, append, draft or evidence_citations")
